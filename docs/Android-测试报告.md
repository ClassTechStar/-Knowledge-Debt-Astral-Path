...
## 八、第二轮：真机问题修复与 13 份教材全量验证（2026-09-23）

### 8.1 真机反馈的问题（用户截图）

在小米真机上出现：

1. 起点页「系统状态」显示 `API 不可用：Failed to fetch。请打开 http://127.0.0.1:5190/`；
2. 藏书阁选中 5 个 PDF（129.4 MB）点击「上传并解析」→ `上传失败：Failed to fetch`。

### 8.2 根因

**APK 把后端地址写死为 `http://127.0.0.1:5190`**。真机脱离 USB（未执行
`adb reverse tcp:5190 tcp:5190`）时，`127.0.0.1` 指向**手机自身**，因此全部请求失败；
且界面只给出"请打开 127.0.0.1:5190"的提示，用户无从配置 → 上传/诊断/账户等**所有联网功能**均不可用。

原候选地址仅 `location.origin` / `127.0.0.1:5190` / `localhost:5190`，**没有用户可配置入口**。

### 8.3 修复

| 项 | 内容 |
|---|---|
| 共享界面（三端通用） | 「系统状态」卡片新增**服务器地址**设置：输入框 + 保存并测试 / 自动探测 / 恢复默认；写入 `localStorage` 持久化；`resolveApiBase` 优先使用用户配置；候选顺序补入 `10.0.2.2:5190`（Android 模拟器宿主映射） |
| 输入归一化 | 未写协议自动补 `http://`；未写端口自动补 `:5190`；去尾部斜杠 |
| 失败可诊断 | 探测结果逐条列出（✅/❌）+ 每个候选地址；首页报错文案改为可操作指引（填局域网 IP 或执行 `adb reverse`） |
| 后端可达性 | 启动横幅打印**手机可填写的局域网地址**；文档补充「同一 Wi-Fi / USB reverse / 手机热点」三种方式与防火墙命令 |

> 该修复随 `index.html` 一并生效于 Web / Windows / Android 三端，无需各自改代码。

### 8.4 同一轮修复的其他缺陷

| # | 缺陷 | 影响 | 修复 |
|---|---|---|---|
| 1 | 上传时把**净化后的文件名同时用作显示标题** | 教材名中的 `[ ] ( ) , +` 被替换为 `_`（如「…(明日科技)…」→「…_明日科技_…」），13 份测试文件中多数命中，用户会误判为缺陷 | 显示标题保留原始文件名（仅剥离路径），磁盘文件名另做安全净化 |
| 2 | 测试脚本用 `curl -F` 上传含**逗号**的文件名 | curl 把逗号当多文件分隔符 → HTTP=000（非产品缺陷，真实端用 FormData 不受影响） | 新增 `scripts/materials_upload.py`（标准库 multipart，与浏览器/WebView 同路径）作为复测工具 |
...
|---|---|
| 通过数 | **13 / 13** |
| 累计页数 / 字符 | 6,720 页 / 4,015,232 字 |
| 累计知识点节点 / 图边 | 2,519 / 2,662 |
| 累计教材任务 / 章节真题 | 78 / 96 |
| 总耗时 | 558 秒（单册 20–161 秒） |
| 最大文件 | Go语言从入门到精通.pdf 120.4 MB → 成功，506 页 / 247 节点 / 258 边 |
| OCR 路径 | 《大模型应用开发：动手做 AI Agent》触发 `ocrUsed=true`，161 秒完成解析 |

逐文件数据见 `docs/materials-13pdf-测试报告.md` 与原始结果 `docs/materials-13pdf-results.tsv`。

### 8.6 本轮暴露的已知限制（如实标注）

| 限制 | 说明 | 影响与建议 |
|---|---|---|
| **教材索引用内存态**（`MaterialRegistry` 为静态字典） | 后端重启后 `/v1/materials` 会清空，已上传文件仍在磁盘（`materials-uploads/`） | 演示场景可接受；若需长期保留，应把注册表落库（可复用 `src/AstralPath.Persistence` 的 Postgres 实现） |
| 真机端到端仍未闭环 | 小米设备的 USB 安装策略与网络环境（VPN / 蜂窝）导致无法自动完成 | 需要在手机上开启「USB 调试（安全设置）」并按 §6 配置服务器地址后人工复测 |

### 8.7 回归验证

```text
dotnet test AstralPath.slnx -c Release
Eval.Tests          2 / 2
Desktop.Tests      47 / 47
Persistence.Tests   8 / 8
Api.Tests          53 / 53
Core.Tests         11 / 11
合计              121 / 121   全部通过
```
